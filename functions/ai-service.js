...
module.exports = function createAiServices(db) {
  ...
  async function generate(system, data) {
    ...
    const transientStatus = status => status === 408 || status >= 500;
    ...
    for (let attempt = 1; attempt <= 2; attempt += 1) {
      ...
      console.error('Gemini API request failed',{status:response.status,model:selectedModel,attempt,details});

      if (attempt === 1 && transientStatus(response.status)) {
