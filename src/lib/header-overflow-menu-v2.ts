...
const ensureStyle = () => {
  ...
  style.textContent = `
    ...
    #${PLAYER_ID} { height:1.5rem;min-height:1.5rem;width:min(100%,29rem);margin-left:auto;margin-right:auto;display:flex;align-items:center;gap:.25rem;padding:0 .25rem 0 .55rem;border:1px solid hsl(var(--border));border-radius:9999px;background:hsl(var(--card) / .82);color:hsl(var(--card-foreground));box-shadow:0 1px 5px rgb(0 0 0 / .08);overflow:hidden; }
    #${PLAYER_ID} .twinly-player-track { min-width:0;flex:1;display:flex;align-items:center;gap:.35rem;font:600 11px/1 'DM Sans','Noto Sans JP',sans-serif;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;border:0;background:transparent;color:inherit;text-align:left;cursor:pointer; }
    #${PLAYER_ID} .twinly-player-track svg { width:.85rem;height:.85rem;flex:0 0 auto; }
    #${PLAYER_ID} .twinly-player-control { width:1.35rem;height:1.35rem;flex:0 0 1.35rem;display:grid;place-items:center;border:0;border-radius:9999px;background:transparent;color:inherit;cursor:pointer;transition:background-color 120ms ease,transform 120ms ease; }
...
const ensurePlayer = (header: HTMLElement) => {
  ...
  if (!player) {
    ...
    player.id = PLAYER_ID;
    player.hidden = true;
    player.innerHTML = `<button type="button" class="twinly-player-track" aria-label="おやすみ音楽を開く">${icons.music}<span></span></button><button type="button" class="twinly-player-control" data-action="previous" aria-label="前の曲">${icons.previous}</button><button type="button" class="twinly-player-control twinly-player-toggle" data-action="toggle" aria-label="一時停止">${icons.pause}</button><button type="button" class="twinly-player-control" data-action="next" aria-label="次の曲">${icons.next}</button>`;
    hint.before(player);
...
const updatePlayer = () => {
  ...
  if (hint.hidden !== comfortState.active) hint.hidden = comfortState.active;
  if (player.hidden === comfortState.active) player.hidden = !comfortState.active;
  ...
  if (toggle) {
    const pausedValue = String(comfortState.paused);
    if (toggle.dataset.paused !== pausedValue) {
      toggle.dataset.paused = pausedValue;
      toggle.innerHTML = comfortState.paused ? icons.play : icons.pause;
    }
    const nextAria = comfortState.paused ? '再生' : '一時停止';
    if (toggle.getAttribute('aria-label') !== nextAria) toggle.setAttribute('aria-label', nextAria);
    if (toggle.title !== nextAria) toggle.title = nextAria;
  }
};
