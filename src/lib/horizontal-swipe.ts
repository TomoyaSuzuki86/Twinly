export type SwipePoint = {
  x: number;
  y: number;
};

export type HorizontalSwipeDirection = "left" | "right";
export type HorizontalSwipeOptions = {
  minDistancePx?: number;
  horizontalDominanceRatio?: number;
};

const MIN_SWIPE_DISTANCE_PX = 56;
const HORIZONTAL_DOMINANCE_RATIO = 1.25;

export const detectHorizontalSwipe = (
  start: SwipePoint,
  end: SwipePoint,
  options: HorizontalSwipeOptions = {}
): HorizontalSwipeDirection | null => {
  const deltaX = end.x - start.x;
  const deltaY = end.y - start.y;
  const horizontalDistance = Math.abs(deltaX);
  const verticalDistance = Math.abs(deltaY);
  const minDistancePx = options.minDistancePx ?? MIN_SWIPE_DISTANCE_PX;
  const horizontalDominanceRatio = options.horizontalDominanceRatio ?? HORIZONTAL_DOMINANCE_RATIO;

  if (horizontalDistance < minDistancePx) return null;
  if (horizontalDistance < verticalDistance * horizontalDominanceRatio) return null;

  return deltaX < 0 ? "left" : "right";
};