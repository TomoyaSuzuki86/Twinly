  ...
  return (
    ...
    >
      ...
      <div
        role="menu"
        data-open={open}
        className={`absolute right-0 top-[calc(100%+0.35rem)] z-[85] w-46 origin-top-right rounded-[0.8rem] border bg-card/95 p-1.5 text-card-foreground shadow-2xl backdrop-blur transition duration-150 ${
      ...
      >
        <button
          ...
          title={aiEnabled ? "AIアドバイス" : aiLoading ? "利用状態を確認しています" : "Premiumで利用できます"}
        ...
        >
          ...
          <span>AIアドバイス</span>
          {!aiLoading && !aiEnabled ? (
            <span className="ml-auto rounded-full bg-primary/10 px-1.5 py-0.5 text-[9px] font-extrabold text-primary">Premium</span>
          ) : null}
        </button>
        <button
          type="button"
...
export function ComfortMiniPlayer() {
  const [state, setState] = useState<ComfortState>({ active: false, paused: false, trackId: "", trackLabel: "" });

  useEffect(() => {
    const handleState = (event: Event) => {
      const detail = (event as CustomEvent<ComfortState>).detail;
      if (detail) setState(detail);
    };
    window.addEventListener("twinly-comfort-state", handleState);
    const timer = window.setTimeout(() => window.dispatchEvent(new Event("twinly-comfort-state-request")), 100);
    return () => {
  ...
  return (
    <div
      className="twinly-comfort-mini-player mx-auto flex h-[1.15rem] min-h-[1.15rem] w-[min(100%,29rem)] items-center gap-1 overflow-hidden rounded-full border bg-card/80 pl-2 pr-1 text-card-foreground shadow-sm"
      onPointerDown={stopHeaderGesture}
      onDoubleClick={stopHeaderGesture}
    >
      <button
