...
type ConflictResolver = {
  resolveConflict: (conflictId: string, side: ConflictSide) => void;
};
...
const renderConflictPanel = (conflicts: SyncConflict[], resolver: ConflictResolver | null) => {
  ...
  const addChoice = (side: ConflictSide, label: string, value: unknown) => {
    ...
    button.addEventListener("click", () => resolver.resolveConflict(conflict.id, side));
...
export const applyAppStorePresentation = (status: StoreStatus, resolver: ConflictResolver | null) => {
  ...
  const showChecking = Boolean(status.ready && status.checking && !status.error && status.pending === 0);
  if (showChecking) document.documentElement.dataset.twinlySyncChecking = "true";
  ...
  renderConflictPanel(status.conflicts ?? [], resolver);
